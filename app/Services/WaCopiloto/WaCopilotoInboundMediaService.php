 ...
 */
class WaCopilotoInboundMediaService
{
    /**
    ...
    public function downloadAndStore(array $msg, $conversationId)
    {
        ...
        $token = trim((string) config('meta_whatsapp_copiloto.access_token'));
        if ($token === '') {
            WaCopilotoLog::warning('inboundMedia.no_token');

            return null;
        }

        $version = trim((string) config('meta_whatsapp_copiloto.graph_api_version', 'v19.0'));
        ...
        try {
            ...
            if (!$meta->successful()) {
                WaCopilotoLog::warning('inboundMedia.meta_failed', [
                    'media_id' => $mediaId,
                    'status' => $meta->status(),
                ]);
            ...
            if ($sizeBytes > 0 && !$this->withinSizeLimit($parsed['kind'], $sizeBytes)) {
                WaCopilotoLog::warning('inboundMedia.too_large', [
                    'kind' => $parsed['kind'],
                    'size' => $sizeBytes,
                ]);
            ...
            if (!$fileRes->successful()) {
                WaCopilotoLog::warning('inboundMedia.download_failed', [
                    'media_id' => $mediaId,
            ...
            $storageKey = CoordinacionMediaLink::buildCopilotoInboundStorageKey(
                $conversationId,
                $parsed['kind'],
                $parsed['filename']
            );
            ...
            WaCopilotoLog::info('inboundMedia.ok', [
            ...
            WaCopilotoLog::error('inboundMedia.exception', [
                'media_id' => $mediaId,
                'error' => $e->getMessage(),
            ]);
