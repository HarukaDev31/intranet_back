...
class ContenedorController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            return response()->json($this->buildContenedorIndexPayload($request, $effectiveRole));
        } catch (\Exception $e) {
        ...
        try {
            ...
            switch ($role) {
                ...
                default:
                    ...
                    if ($authOrg !== Usuario::ID_ORGANIZACION_ADMIN) {
                        $query->where('tipo', 'COTIZADOR')->limit($request->boolean('completado') ? 3 : 2);
                    } else {
                        $query->where('tipo', 'COTIZADOR');
                    }
                    break;
    ...
    */
    ...
    private function crearNotificacionesMovimientoConsolidado($cotizacion, $idContenedorOrigen, $idContenedorDestino)
    {
        try {
            ...
            return [$notificacionCoordinacion, $notificacionJefeVentas, $notificacionCotizador, $notificacionRRHH];
        } catch (\Exception $e) {
