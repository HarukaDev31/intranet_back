...
class CreateBdProductosRegulacionesEtiquetadoMediaTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('bd_productos_regulaciones_etiquetado_media', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('id_regulacion')->comment('ID de la regulación de etiquetado');
            ...
            // Foreign key
            $table->foreign('id_regulacion', 'fk_etiquetado_media_regulacion')
                  ->references('id')
                  ->on('bd_productos_regulaciones_etiquetado')
                  ->onDelete('cascade')
                  ->onUpdate('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('bd_productos_regulaciones_etiquetado_media');
    }
}
