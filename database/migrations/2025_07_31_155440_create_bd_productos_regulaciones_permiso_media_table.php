...
class CreateBdProductosRegulacionesPermisoMediaTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('bd_productos_regulaciones_permiso_media', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('id_regulacion')->comment('ID de la regulación de permiso');
            ...
            // Foreign key
            $table->foreign('id_regulacion', 'fk_permiso_media_regulacion')
                  ->references('id')
                  ->on('bd_productos_regulaciones_permiso')
                  ->onDelete('cascade')
                  ->onUpdate('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('bd_productos_regulaciones_permiso_media');
    }
}
